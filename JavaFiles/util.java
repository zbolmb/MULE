...
public class util {
    ...
    public static String applyRandomEvent() {
        ...
        Player cp = Configurations.curPlayer;
        int money = randomEvents[rand].money;
        int food = randomEvents[rand].food;
        int energy = randomEvents[rand].energy;
        
        if (playerOrder.size() == 0) {
            while (money < 0 || food < 0 || energy < 0) {
                rand = (int) (Math.random() * randomEvents.length);
                money = randomEvents[rand].money;
                food = randomEvents[rand].food;
                energy = randomEvents[rand].energy;
            }
        }
