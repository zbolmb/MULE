...
public class GameMap {
    ...
    public void addMule(int x, int y, int m) {
        if (!dc.map.aMap[x][y].getMule(m)) {
            dc.map.aMap[x][y].setMule(m);
            Rectangle mule = new Rectangle(x * 100 + 20 + (20 * m), y * 100 + 55, 15, 33);
            Text t = new Text(x * 100 + 20 + (20 * m) + 4, y * 100 + 55 + 25, "F");
            if (m == 0) t.setText("F");
            if (m == 1) t.setText("E");
            if (m == 2) t.setText("O");
            if (Configurations.curPlayer.color == Color.BLACK) t.setFill(Color.WHITE);
            mule.setFill(Configurations.curPlayer.color);
            dc.mapGUI.getChildren().addAll(mule, t);
        }
    }
