...
public class util {
    ...
    static DisplayContents dc = Configurations.getDisplayContents();
    ...
    private static void buyTurnIncre() throws IOException {
        ...
        if (Configurations.getCurPlayer().isPassed()) buyTurnIncre();
    ...
    private static void movePhaseTurnIncre() throws IOException {
        if (playerOrder.isEmpty()) {
            ...
            Configurations.setRound(Configurations.getRound() + 1);
            if (Configurations.getPhase() == 1) produce();
        }
        Configurations.setCurPlayer(playerOrder.remove());
    ...
    public static void addMovementHandlers(Scene scene) {
        scene.addEventHandler(KeyEvent.KEY_PRESSED, k -> {
            if (k.getCode() == KeyCode.LEFT) dc.getLoopService().move.setL(-5);
            if (k.getCode() == KeyCode.RIGHT) dc.getLoopService().move.setR(5);
            if (k.getCode() == KeyCode.UP) dc.getLoopService().move.setU(-5);
            if (k.getCode() == KeyCode.DOWN) dc.getLoopService().move.setD(5);
        });
        scene.addEventHandler(KeyEvent.KEY_RELEASED, k -> {
            if (k.getCode() == KeyCode.LEFT) dc.getLoopService().move.setL(0);
            if (k.getCode() == KeyCode.RIGHT) dc.getLoopService().move.setR(0);
            if (k.getCode() == KeyCode.UP) dc.getLoopService().move.setU(0);
            if (k.getCode() == KeyCode.DOWN) dc.getLoopService().move.setD(0);
        });
    }
