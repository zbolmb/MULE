...
public class LoopService extends AbstractLoopService {
        ...
        if (Configurations.getPhase() == 1) {
            ...
            if (cp.getPhase() == 0) {
                ...
                if (cp.getX() < 500 && cp.getX() > 400 && cp.getY() < 300 && cp.getY() > 200) {
                    ...
                    //lose all your mules
                    if (!map[x][y].getOwner().equals(cp.getName()) && !map[x][y].getOwner().equals("None")) {
                        cp.setMule1(0);
                        cp.setMule2(0);
                        cp.setMule3(0);
                    } else {
                        ...
                        if (cp.getMule1() > 0 && map[x][y].getOwner().equals(cp.getName())) {
                            cp.setMule1(cp.getMule1() - 1);
                            dc.getMap().addMule(x, y, 0);
                        }
                        if (cp.getMule2() > 0 && map[x][y].getOwner().equals(cp.getName())) {
                            cp.setMule2(cp.getMule2() - 1);
                            dc.getMap().addMule(x, y, 1);
                        }
                        if (cp.getMule3() > 0 && map[x][y].getOwner().equals(cp.getName())) {
                ...
                if (cp.getX() < 5 || cp.getX() > 795 || cp.getY() < 5 || cp.getY() > 495) {
                    ...
                    cp.setScore((int) (cp.getScore() + (50 + rng * (120 - time))));
                    cp.setMoney((int) (cp.getMoney() + (50 + rng * (120 - time))));
                    time = 0;
                    util.next();
                    move.setL(0);
                    move.setR(0);
    ...
    protected void runInBackground() {
        ...
        if (Configurations.getPhase() == 1 || Configurations.getPhase() == 2) {
            vx = move.getL() + move.getR();
            vy = move.getU() + move.getD();
            if (vx == 5) move.setDir('r');
            if (vx == -5) move.setDir('l');
